...
class _LoginScreenState extends State<LoginScreen> with TickerProviderStateMixin {
  ...
  Future<void> _cargarIdServicio() async {
    final id = await DeviceService().obtenerIdServicio();
    if (mounted) setState(() => _idServicio = id);
  }
  ...
  void _copiarId(String id) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
      content: Text('ID de servicio copiado'),
      duration: Duration(seconds: 1),
    ));
  }

  void _mostrarDialogoActivacion(String id) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            const Expanded(child: Text('Dispositivo no autorizado')),
          ],
        ),
        content: Column(
          ...
          children: [
            const Text(
              'Envía este ID de servicio a Soporte TI para que active tu dispositivo:',
                  ...
                  children: [
                    Text(id,
                        style: const TextStyle(
                            ...
                            letterSpacing: 1)),
                    const SizedBox(width: 10),
                    Icon(Icons.copy_rounded, size: 18, color: _accent),
                  ],
                ),
              ),
            ),
  ...
  Widget _buildIdServicioFooter() {
    final id = _idServicio ?? '';
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.qr_code_2_rounded, size: 18, color: Colors.grey.shade500),
            const SizedBox(width: 10),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
